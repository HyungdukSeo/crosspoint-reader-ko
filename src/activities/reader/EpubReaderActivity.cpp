...
void EpubReaderActivity::openReaderMenu() {
  ...
  const uint8_t menuHyphenation = SETTINGS.hyphenationEnabled;
  ...
  startActivityForResult(
      std::make_unique<EpubReaderMenuActivity>(renderer, mappedInput, epub->getTitle(), currentPage, totalPages,
                                               ...
                                               readingTimer.totalSeconds(), !cachedBookmarks.empty()),
      [this, menuOrientationSnapshot, menuLineSpacing, menuScreenMargin, menuParagraphAlignment, menuEmbeddedStyle,
       ...
       menuHyphenation, menuFontId](const ActivityResult& result) {
        const auto& menu = std::get<MenuResult>(result.data);

        // Reconcile orientation: the menu's pending rotate-screen cycle and Reader Options' direct
        // write both feed the same setting. Pick whichever differs from the snapshot (menu cycle
        // wins ties so the existing rotate-screen UX is preserved).
        ...
        // applyOrientation() expects the SETTING to still hold the OLD value — restore the snapshot
        // first so the helper does its full re-layout when the orientation actually changed.
        SETTINGS.orientation = menuOrientationSnapshot;
        applyOrientation(finalOrientation);
        toggleAutoPageTurn(menu.pageTurnOption);

        // applyOrientation() already invalidates the section when it changes orientation. For other
        // layout-affecting settings, detect the change here and drop the in-memory section so the
        // next render re-checks the cache against the new spec.
